<?php @session_start();
/* first: 25/09/2017 last :07/09/2018 YGPC modified 
...
*/
define('_SERVICE' , 00 ); // 10 SHQ 
define('_Gmach_Mail' , "gmachexpress@gmail.com" ); 
define('_CARENCE_MONTH' , 6 ); //  6-> 180  ימים המתנה אחרי סוף הלוואה קודמת 
    ...
    // test if download debt doc  request 
        ...
        include( "././compteur/index.php" ); // count of downloads      
	          ...
	          //	$img_exists  =	$this-> _saveIdentityImg() ; 
	          	$img_exists  = $this-> _saveDoc('identity','tz') ; 
	          	$img_debt_exists  =	$this-> _saveDebtImg() ;
	          	$img_address_proof_exists  =	$this-> _saveAddressProofImg() ;
	            $img_pay_slip_exists  =	$this->_savePaySlipImg() ;
            
	         	//        var_dump($img_exists  ,$img_debt_exists ,$img_address_proof_exists ,	$img_pay_slip_exists  );exit;

	            if(   !$img_exists   || !$img_debt_exists ||  !$img_address_proof_exists || !$img_pay_slip_exists )  // upload or change image
	          	{    //exit ('img') ;
            ...
            case OrderController::STEP_PAYMENT:
             ...
             //	$img_exists  =	$this-> _saveIdentityImg() ; 
                	...
                	$img_address_proof_exists  =	$this->_saveAddressProofImg() ;
               	$img_pay_slip_exists  =	$this->_savePaySlipImg() ;
	    //    var_dump($img_exists  ,$img_debt_exists ,$img_address_proof_exists ,	$img_pay_slip_exists,$cgv  );exit;
	           if(   !$img_exists   || !$img_debt_exists ||  !$img_address_proof_exists || !$img_pay_slip_exists || !$cgv)  // upload or change image
	          	{ 	
	          		 Tools::redirect('index.php?controller=order&step=2');
	          	}
               ...
               // $available_tz = $this->_testTz();
                $available_tz = $this->_test_img( 'identity' );	// test if there is an image of tz
				...
				if( $this->_validate( $available_tz ) )   
					{
						    ...
						    $available_pay_slip = FALSE;
            				    $available_pay_slip = $this->_testPay_Slip();	// test if there is an image of address proof
            				  if ( !$available_pay_slip )
							{
			              		 	 echo ' <script type=text/javascript>alert("בבקשה נא להכניס תמונה של  תלוש משכורת !")</script></Div>';
			              		 	 $this->setTemplate(_PS_THEME_DIR_.'order-carrier.tpl'); // loop to the bank form
			              	 	 	return;		             	 	 
							}
...
//  TOS = TERM OF SERVICE (Read the Terms of Service)
 ...
 /*   public function WordtoHtml( $wordfile )
   ...
   protected function  _saveDoc( $name, $up_name ) 
    {
		        if (!isset($this->context->customer->id)) {
		            die(Tools::displayError('Fatal error: No customer'));
		        }
		       if(   $_POST['change_'.$name])
		       	{
		       				unset( $_SESSION[ $name .'_img'] );
							unset( $_SESSION['uploadrealfile_'. $name] );	
		       		 return FALSE ;
		      	}
		        
			$uploaddir = '../../upload/'.$up_name.'/';// image DIR 
			$id_customer = (int)$this->context->customer->id ;	
			if( file_exists ( $_FILES[ $name.'_img']['tmp_name'] ) ) // image was uploaded
					{	 
						 $uploadfile =  $uploaddir .$id_customer; // image file name with id_customer in it             ../../upload/$name/1
						 
						$uploadfile .= '_'.basename(  $_FILES[$name.'_img']['name'] ) ; // name without path         ../../upload/$name/1_img_name.jpg
						$uploadrealfile = _PS_ROOT_DIR_."/upload/".$up_name."/".$id_customer.'_'.basename(  $_FILES[$name.'_img']['name'] ) ;	
						
						$this->context->smarty->assign( array( $name. '_img_name' => $_FILES[$name.'_img']['name']  ) );
						$this->context->smarty->assign( array(  $name.'_img_path' =>  $uploaddir   ) ) ;
						$this->context->smarty->assign( array( $name.'_img_real_name' => $uploadfile ) ) ;
						...
						if (  move_uploaded_file( $_FILES[$name.'_img']['tmp_name'], $uploadrealfile )  ) // copy the tmp file to the real path 
						  {
						
							$_SESSION[$name.'_img'] =$uploadfile;
							$_SESSION['uploadrealfile_'.$name] =$uploadrealfile;	
							}
					 	$_SESSION[$name.'img_tmp_name'] =   $name.'_img';
					 	unset( $_FILES[$name.'_img']['tmp_name']  );
					 	unset($_FILES[$name.'_img']['name'] );
					 	return( TRUE );
			    	  }
			      else    //if  image not uploaded -> we take the one in cache 
			   	   {
			   	   	   
			   	   	   if( $_SESSION['uploadrealfile_'.$name] !== ''  )
			   	   	   	{	
			   	   	   	 $identity_img = $_SESSION[$name.'_img'] ;
			   	   	   	 $this->context->smarty->assign( 
									 array( 	$name.'_img_name' => str_replace ( $id_customer.'_' , '' , basename( $identity_img )  ),
									 	 	$name.'_img_path' =>  $uploaddir,
									 	 	$name.'_img_real_name' => $identity_img
									  )  );
						unset( $_FILES[$name.'_img']['tmp_name']  );
					 	unset($_FILES[$name.'_img']['name'] );
						return( TRUE );			   	   	   
		    		  	...
		    		  	$sql =  " ";
					$sql =  " SELECT ".$name."_img FROM admin_gmahexpress.ko_message WHERE id_customer = '".$id_customer."'  LIMIT 1" ;
						
					 $img_exists  = Db::getInstance()->executeS( $sql  );
				
					if ( $img_exists[0][$name.'_img']  !== '' ) 
							{	
								 $this->context->smarty->assign( 
								 array( 	$name.'_img_name' => str_replace ( $id_customer.'_' , '' , basename( $img_exists[0][$name.'_img'] )  ),
								 	 	$name.'_img_path' =>  $uploaddir,
								 	 	$name.'_img_real_name' => $uploaddir.basename( $img_exists[0][$name.'_img'] )  
								  )  );
								 $_SESSION['uploadrealfile_'.$name] = _PS_ROOT_DIR_."/upload/".$name."/".
								 								$id_customer.'_'.basename( $img_exists[0][$name.'_img'] ) ;	

								 $_SESSION[$name.'_img'] =$img_exists[0][$name.'_img'];	
		       ...
		       if(   $_POST['change'])
		       	{
							...
							unset( $_SESSION['uploadrealfile_identity'] );	
		       		 return FALSE ;
		      	}
			...
			if( file_exists ( $_FILES['identity_img']['tmp_name'] ) ) // image was uploaded
					{	 
						...
						if (  move_uploaded_file( $_FILES['identity_img']['tmp_name'], $uploadrealfile )  ) // copy the tmp file to the real path 
						  {
							...
							$_SESSION['uploadrealfile_identity'] =$uploadrealfile;	
							}
			      ...
			      else    //if  image not uploaded -> we take the one in cache 
			   	   {
			   	   	   ...
			   	   	   if( $_SESSION['uploadrealfile_identity'] !== ''  )
			   	   	   	{	
					...
					if ( $img_exists[0]['identity_img']  !== '' ) 
							{	
								 ...
								 $_SESSION['uploadrealfile_identity'] = _PS_ROOT_DIR_."/upload/tz/".
   ...
   protected function  _saveDebtImg() 
    {
			...
			if( file_exists ( $_FILES['debt_img']['tmp_name'] ) ) // image was uploaded
					{	
						...
						$uploadfile .= '_'. strtolower( basename(  $_FILES['debt_img']['name'] )  ); // name without path         ../../upload/debt/1_img_name.jpg
						$uploadrealfile = _PS_ROOT_DIR_."/upload/debt/".$id_customer.'_'. strtolower( basename(  $_FILES['debt_img']['name'] ) ) ;	
  ...
  /*/////////////////////_savePaySlipImg////////////////////////////////*/
/* don t forget to build the folder !*/
   protected function  _savePaySlipImg() 
    {
		        if (!isset($this->context->customer->id)) {
		            die(Tools::displayError('Fatal error: No customer'));
		        }
		       if(   $_POST['change_pay_slip'] )
		       	{
		       		unset( $_SESSION['pay_slip_img'] );
					unset( $_SESSION['uploadrealfile_pay_slip'] ) ;
		       		 return FALSE ;
		      	}
		        
			$uploaddir = '../../upload/pay_slip/';//  address image DIR 
			$id_customer = (int)$this->context->customer->id ;	
			if( file_exists ( $_FILES['pay_slip_img']['tmp_name'] ) ) // image was uploaded
					{
						 $uploadfile =  $uploaddir .$id_customer; // image file name with id_customer in it             ../../upload/address_proof/1
						 
						$uploadfile .= '_'.basename(  $_FILES['pay_slip_img']['name'] ) ; 
							// name without path         ../../upload/address_proof/1_img_name.jpg
						$uploadrealfile = _PS_ROOT_DIR_."/upload/pay_slip/".$id_customer.'_'.basename(  $_FILES['pay_slip_img']['name'] ) ;	
						
						$this->context->smarty->assign( array( 'pay_slip_img_name' => $_FILES['pay_slip_img']['name']  ) );
						$this->context->smarty->assign( array(  'pay_slip_img_path' =>  $uploaddir   ) ) ;
						$this->context->smarty->assign( array( 'pay_slip_img_real_name' => $uploadfile ) ) ;
						// smarty  need  $uploadfile relative path it  don't work with full path $uploadrealfile 
						// move_uploaded_file need $uploadrealfile  it don't work with relative path 	$uploadfile
						//var_dump( $_FILES['pay_slip_img']['tmp_name'] ,' -- ',$uploadrealfile); //exit;
						if (  move_uploaded_file( $_FILES['pay_slip_img']['tmp_name'], $uploadrealfile )  ) // copy the tmp file to the real path 
						  {
						//	var_dump( $_FILES['pay_slip_img']['tmp_name'] ,' -- ',$uploadrealfile); exit;
							$_SESSION['pay_slip_img'] =$uploadfile;
							$_SESSION['uploadrealfile_pay_slip'] =$uploadrealfile;	
						  }
					 		$_SESSION['img_pay_slip_tmp_name'] =   'pay_slip_img';
					 	unset( $_FILES['pay_slip_img']['tmp_name']  );
					 	unset($_FILES['pay_slip_img']['name'] );
					 	return( TRUE );
			    	  }
			      else    //if  image not uploaded -> we take the one in cache 
			   	   {
			   	   	 
			   	   	   if( $_SESSION['uploadrealfile_pay_slip'] !== ''  )
			   	   	   	{	
			   	   	   		   //exit( ' savepay_slipImg ' );
			   	   	   	 $pay_slip_img = $_SESSION['pay_slip_img'] ;
			   	   	   	 $this->context->smarty->assign( 
									 array( 	'pay_slip_img_name' => str_replace ( $id_customer.'_' , '' , basename( $pay_slip_img )  ),
									 	 	'pay_slip_img_path' =>  $uploaddir,
									 	 	'pay_slip_img_real_name' => $pay_slip_img
									  )  );
						unset( $_FILES['pay_slip_img']['tmp_name']  );
					 	unset($_FILES['pay_slip_img']['name'] );
						return( TRUE );			   	   	   
			   	   	    ...
			   	   	    //if  image not uploaded yet -> we take the one in db 
		    		  	$sql =  " ";
					$sql =  " SELECT pay_slip_img FROM admin_gmahexpress.ko_message WHERE id_customer = '".$id_customer."'  LIMIT 1" ;
						
					 $img_exists  = Db::getInstance()->executeS( $sql  );
				
					if ( $img_exists[0]['pay_slip_img']  !== '' ) 
							{	
								 $this->context->smarty->assign( 
								 array( 	'pay_slip_img_name' => str_replace ( $id_customer.'_' , '' , basename( $img_exists[0]['pay_slip_img'] )  ),
								 	 	'pay_slip_img_path' =>  $uploaddir,
								 	 	'pay_slip_img_real_name' => $uploaddir.basename( $img_exists[0]['pay_slip_img'] )  
								  )  );
								 $_SESSION['uploadrealfile_pay_slip'] = _PS_ROOT_DIR_."/upload/pay_slip/".
								 $id_customer.'_'.basename( $img_exists[0]['pay_slip_img'] ) ;	

								 $_SESSION['pay_slip_img'] =$img_exists[0]['identity_img'];	
								 //	var_dump (__LINE__,	$_SESSION['pay_slip_img']  )  ;	exit;
								 	unset( $_FILES['pay_slip_img']['tmp_name']  );
					 				unset($_FILES['pay_slip_img']['name'] );			
								    	return( TRUE );	  
		    					}
   ...
   protected function _saveBankAccount()
		...
		if ( isset ( $_POST['bank_name'] ) )
			{
				...
				$_SESSION['bank_name'] =addslashes ($_POST['bank_name'] );
				$_SESSION['account_number'] =	addslashes ($_POST['account_number']);
				$_SESSION['bank_number']	 = addslashes ($_POST['bank_number']);
				$_SESSION['sucursale'] =addslashes ($_POST['sucursale']); 
				$_SESSION['bank_address1']=addslashes ($_POST['bank_address1']);
				$_SESSION['bank_postcode'] =addslashes ($_POST['bank_postcode']);
				$_SESSION['bank_CityName']=addslashes ($_POST['bank_CityName']);
				$_SESSION['bank_country']= addslashes ($_POST['bank_country']);
				$_SESSION['bank_phone']=addslashes ($_POST['bank_phone']);
				$_SESSION['identity_number']	 = addslashes ($_POST['identity_number']);
				$_SESSION['message']	 = addslashes ($_POST['message']);
				$_SESSION['work']	 = addslashes ($_POST['work']) ;
				$_SESSION['cgv']	 = addslashes ($_POST['cgv']);
				$_SESSION['other_loans']	 = $_POST['other_loans'];
				$this->context->smarty->assign( array( 'bank_name' => $_POST['bank_name'], 'account_number'=>  $_POST['account_number'] ,'bank_number' =>$_POST['bank_number'], 'sucursale' =>$_POST['sucursale']  ,  'bank_address1' =>$_POST['bank_address1'] ,   'bank_postcode' =>$_POST['bank_postcode'] ,  'bank_CityName' =>$_POST['bank_CityName'] ,  'bank_country' =>$_POST['bank_country'] ,  'bank_phone' =>$_POST['bank_phone'] , 'identity_number' =>$_POST['identity_number'] , 
				 'reason' =>$_POST['message'] ,  'business' =>$_POST['work'] ,'cgv' =>$_POST['cgv'] ,'other_loans' =>$_POST['other_loans'] ) );
			//	 exit (__LINE__) ;
		}
		else
		{
			...
			{	//exit (__LINE__) ;
				 $this->context->smarty->assign( array( 'bank_name' => $_SESSION['bank_name'], 
					 ...
					 'sucursale' =>$_SESSION['sucursale']  ,  'bank_address1' =>$_SESSION['bank_address1'] ,  
					  ...
					  'identity_number' =>$_SESSION['identity_number'] ,  'reason' =>$_SESSION['message'] , 
					 'business' =>$_SESSION['work'] ,
					  'cgv' =>$_SESSION['cgv']  ) );
			}
		}	
    ...
    public function _test_img( $name )
    {
  
        if( !$_SESSION['uploadrealfile_'.$name] ) //   if( $result !== 'NULL' )  
	  		{   
     			return (  FALSE ) ;
     		 }
     	else
     		{   	if ( filesize ( $_SESSION['uploadrealfile_'.$name] ) === FALSE  ||  filesize ( $_SESSION['uploadrealfile_'.$name] ) > 4000000   ) 
     				{	echo'<script type=text/Javascript> alert(" תמו נה לא חוקית גודל צריך ליהיות בן 500K ל4MB ");</script>';
     					return( false);
     				}
     			return ( $_SESSION[$name.'_img']  ) ;
     		 }	
        return (  FALSE ) ; 
    }
    ...
    public function _testTz()
    {
       ...
       $result = FALSE; 
    	 ...
    	 */
        if( !$_SESSION['uploadrealfile_identity'] ) //   if( $result !== 'NULL' )  
	  		{   
     			return (  FALSE ) ;
     		 }
     	else
     		{   	if ( filesize ( $_SESSION['uploadrealfile_identity'] ) === FALSE  ||  filesize ( $_SESSION['uploadrealfile_identity'] ) > 4000000   ) 
     				{	echo'<script type=text/Javascript> alert(" תמו נה לא חוקית גודל צריך ליהיות בן 500K ל4MB ");</script>';
     					return( false);
     				}
    ...
    public function  _testPay_slip()
    {
        if( !$_SESSION['uploadrealfile_pay_slip'] ) //   if( $result !== 'NULL' )  
	  		{   
     			return (  FALSE ) ; 
     		}
     	else
     		{   	if ( filesize ( $_SESSION['uploadrealfile_pay_slip'] ) === FALSE  ||  filesize ( $_SESSION['uploadrealfile_pay_slip'] ) > 4000000   ) 
     				{	echo'<script type=text/Javascript> alert(" תמו נה לא חוקית גודל צריך ליהיות בן 500K ל4MB ");</script>';
     					return( false);
     				}
     			return ( $_SESSION['pay_slip_img']  ) ;
     		 }	
        return (  FALSE ) ; 
    } 
     /* ******************************* _validate() ****************************************** */
	...
	if( $result)
	 {
		...
		if(  $result === NULL || $interval->format( '%R%a days' ) >   ( $result[0]['billing_cycles'] +_CARENCE_MONTH )* 30  )  // ADDED YGPC 
	  		  {
				...
				$options['{uploadrealfile_tz}'] = $_SESSION['uploadrealfile_identity'];
				$options['{uploadfile_tz}'] = $_SESSION['identity_img'];
				$options['{uploadrealfile_debt}'] = $_SESSION['uploadrealfile_debt'];
				$options['{uploadfile_debt}'] = $_SESSION['debt_img'];
				...
				$tpl_name='manager_validation';
		            ...
		            $file_attachment['tmp_name_tz'] =  $_SESSION['uploadrealfile_identity'];
   	 	...
   	 	{	// waiting end of loan plus 180days 
   	 		$permission = 	( $result[0]['billing_cycles'] + _CARENCE_MONTH ) * $this->carence  - $interval->format( '%R%a days' ) ; 
