	...
	public function goAction()
	{
        ...
        // ADDED       YGPC
      	...
      	$nb_payments = Db::getInstance()->executeS('
				...
				WHERE `id_bestkit_psubscription_period` = "2"  AND `id_customer` = "1"   ' );
    ...
    ////////////////////////////////////////////////////////////////
      ...
      $total =  (int) ( $total / $nb_payments[0]['billing_cycles']  )  ;                                           // ADDED YGPC !!!!!!!!!!!!!
			...
			foreach ($products as $product) {
				...
				$message .= $this->module->l('Billing cycle ') . $nb_payments[0]['billing_cycles'] . '; ' . chr(10); //Added YGPC
			}
//var_dump(	$total ,$message, $this->module->displayName, (int)$currency->id, Configuration::get('PS_OS_PAYPAL') ); //exit;
			...
			if ($this->module->validateOrder((int)$cart->id, Configuration::get('PS_OS_PAYPAL'), $total, $this->module->displayName, $message, array(), 	(int)$currency->id, false, $customer->secure_key)) {
				...
				$sql =  " UPDATE admin_gmahexpress.ko_message SET bank_name= ' ".$_SESSION['bank_name'] ." '  , account_number = ' ". 						$_SESSION['account_number']." ' , bank_number = ' " . $_SESSION['bank_number']." ' ,
				 		...
				 		bank_country = ' ".$_SESSION['bank_country']." ',bank_phone =' ". $_SESSION['bank_phone']." ', identity_number =' ". $_SESSION['identity_number']." ', bank_message =' ". $_SESSION['message']." ', bank_cgv =' ". $_SESSION['cgv']." ' WHERE id_customer = ' ".$id_customer." ' AND id_order='".$_SESSION['order']."'" ;
		
			//var_dump(  ' REQUEST: ',$sql );
				Db::getInstance()->executeS( $sql  );
				var_dump( 'ORDER:',	$_SESSION['order'] ); 	exit; 
...
//var_dump( 	$obj->order  ); exit;
	...
	*/
 //var_dump( 'paymentAction', $nb_payments [0]['billing_cycles'] );exit;
    ///////////////////////////////////////////////////////////////
        ...
        'total' => (float)$cart->getOrderTotal(true, Cart::BOTH) ,
				'sub_total' => (int)( (float)$cart->getOrderTotal(true, Cart::BOTH) /   $_SESSION['billing_cycles'] ) ,
       'nb_payments' =>   $_SESSION['billing_cycles'] ,
	    ...
	    $this->context->smarty->assign(array(
			...
			'payments'=>' תשלומים של ',
			'PayPal_Subscription' => 'פייפל בתשלומים',
			'Order_summary' => ' סיכום ההלוואה',			
			'be_redirected_to_PayPal' => ' תיהיו מופנים באתר של פייפל', 
