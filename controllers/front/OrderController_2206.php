...
class OrderControllerCore extends ParentOrderController
{
    ...
    public function initContent()
    {
        ...
        switch ((int)$this->step) {
            ...
            case OrderController::STEP_DELIVERY:
       	  ...
       	  if( file_exists (  $_FILES['identity_img']['tmp_name'] ) || isset($_POST['change'] )  )  // upload or change image
	          	 { 
	          	 	$this-> _saveIdentityImg() ; 
	           	}
             //   $this->autoStep();
   ...
   protected function  _saveIdentityImg() 
    {
			...
			$uploaddir = '/var/www/vhosts/konim.biz/ps_gmach/upload/tz/';// image DIR 
			$id_customer = (int)$this->context->customer->id ;	
					if( file_exists ( $_FILES['identity_img']['tmp_name'] ) )
					{	
					
	    					 $this->context->smarty->assign( array( 'identity_img_name' => $_FILES['identity_img']['name']  ) );
	    						 
						 $uploadfile =  $uploaddir .$id_customer; // image file name with is_customer in it 
						 //var_dump ( $id_customer  )  ;	
						 $uploadfile .= '_'.basename(  $_FILES['identity_img']['name'] ) ;
				
						if (  move_uploaded_file( $_FILES['identity_img']['tmp_name'], $uploadfile )  )
							 {
								$result = $dbh -> exec( 'UPDATE  admin_gmahexpress.ko_address SET  identity_img ="'.$uploadfile.'" WHERE id_customer = "'.$id_custumer.'"  ')  ;
   ...
   protected function _saveBankAccount()
    { 
		        ...
		        $dsn = 'mysql:host=localhost;dbname=admin_admin_gmahexpress';
		        $username = 'gmahexp';
		        $password = 'zx262626';
		        $options = array(  PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8',); 
		...
		//	      throw new Exception(' SQL error BankAccount ');
			 /*	$dbh -> exec( " INSERT INTO `ko_adress ` (`bank_name`,`account_number`,`bank_number`,`sucursale`,`bank_address1`,`bank_postcode`,`bank_CityName`,`bank_country`,`bank_phone`,`identity_number`,
		    	 	 `message` ,`accept`,`identity_img` ) 
		    	 	 VALUES( $_POST['bank_name'],$_POST['account_number'],$_POST['bank_number'],$_POST['sucursale'],$_POST['bank_address1'],$_POST['bank_postcode'],$_POST['bank_CityName'],$_POST['bank_country'],$_POST['bank_phone'],
		    	 	 $_POST['identity_number'],$_POST['message']  ,$_POST['accept'], $_POST['identity_img'] ) " );
		  */
		    	     //$dbh -> exec( " INSERT INTO `ko_adress ` ( `identity_img` )     VALUES( 	$uploadfile  ) ")  ;
