...
//var_dump (  '<br/> STEP: ',  (int)$this->step ); //exit;
        ...
        switch ((int)$this->step) {
		    ...
		    {	
				...
				$options['datas'] = array('{nom}'  => $this->context->customer->firstname,  '{prenom}'  => $this->context->customer->lastname ,'{tz}'=>"/var/www/vhosts/konim.biz/ps_gmach/upload/tz/". basename( $available_tz )  );
				$options['dir_tpl']= _PS_MAIL_DIR_;
			
				$result = $this->send_mail( $to, $tpl_name,$options ) ;  // send a mail to say that the ask is processed
//var_dump( $result ); exit;

			   if( $this->validate() )    // test if the director give his agrement  according to the identity card and if there is no current loan active.	
   ...
   protected function  _saveIdentityImg() 
    {
			...
			if( file_exists ( $_FILES['identity_img']['tmp_name'] ) ) // image was uploaded
			      ...
			      else    //if  image not uploaded yet -> we take the one in db 
			   	   {
			   	   	   
			   	   	   if(  $_SESSION['identity_img']  !== ''  )
			   	   	   {
			   	   	   	 $this->context->smarty->assign( 
									 array( 	'identity_img_name' => str_replace ( $id_customer.'_' , '' , basename( $_SESSION['identity_img'])  ),
									 	 	'identity_img_path' =>  $uploaddir,
									 	 	'identity_img_real_name' => $uploaddir.basename( $_SESSION['identity_img'] )  
									  )  );
						//	var_dump(  ' REQUEST: ',$_SESSION['identity_img']);exit;			
									    	return( TRUE );			   	   	   
			   	   	   }
			    		  		$sql =  " ";
...
//var_dump(  ' REQUEST: ',$sql );exit;
    ...
    public function _testTz()
    {
       ...
       //  var_dump(  $result ); exit(' from   _testTz');
        if( !$_SESSION['identity_img'] ) //   if( $result !== 'NULL' )  
	  		{   
     			// var_dump($_SESSION['identity_img']); exit;
     			return (  FALSE ) ; }
     	else
     		{  //  var_dump($_SESSION['identity_img']);exit ('No image ');
