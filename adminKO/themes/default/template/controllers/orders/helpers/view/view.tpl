...
{block name="override_tpl"}
	...
	<div class="row">
		<div class="col-lg-20"> <!--    ADDED YGPC 20 in place to 7 -->
			<div class="panel">
		...
		<div class="col-lg-20">
			<!-- Customer informations -->  <!--    ADDED YGPC 20 in place to 5 -->
			<div class="panel">
				{if $customer->id}
					<div class="panel-heading">
						<i class="icon-user"></i>
				...
				<div class="row">
					...
					{* <!--    ADDED YGPC suppress Shipping address -->
                    	<li class="active">
...
<font color="BROWN" >
  {l s=' בנק: '}    {$bank_name }  <br/>   {l s=' מספר בנק :  '} {$bank_number }  <br/>   {l s=' סניף :'}  {$sucursale} <br/> {l s='   חשבון: '} {$account_number}  
		    <br/>   {l s=' כתובת:  '} 		 {$bank_address1} {$bank_postcode}  {$bank_CityName} 
    <br/>    {l s= ' טל :' }              {$bank_phone} 
                   <br/>      {l s= '    תעודת זאות :   ' } {$identity_number} <br/> 
			...
			geocoder.geocode({
				...
				if (status === google.maps.GeocoderStatus.OK)
				{
					...
					var delivery_marker = new google.maps.Marker({
						...
						url: 'http://maps.google.com?q={$addresses.delivery->address1|urlencode},{$addresses.delivery->postcode|urlencode},{$addresses.delivery->city|urlencode}{if isset($addresses.deliveryState->name) && $addresses.delivery->id_state},{$addresses.deliveryState->name|urlencode}{/if},{$addresses.delivery->country|urlencode}?key=AIzaSyDI9C0g1f4MskCZdjtKY4wG3crrHFIBKy4&callback=initMap'
					});
			...
			geocoder.geocode({
				...
				if (status === google.maps.GeocoderStatus.OK)
				{
					...
					invoice_marker = new google.maps.Marker({
						map: invoice_map,
						position: results[0].geometry.location,
						url: 'http://maps.google.com?q={$addresses.invoice->address1|urlencode},{$addresses.invoice->postcode|urlencode},{$addresses.invoice->city|urlencode}{if isset($addresses.deliveryState->name) && $addresses.invoice->id_state},{$addresses.deliveryState->name|urlencode}{/if},{$addresses.invoice->country|urlencode}?key=AIzaSyDI9C0g1f4MskCZdjtKY4wG3crrHFIBKy4&callback=initMap'
					});
