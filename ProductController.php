...
class ProductControllerCore extends FrontController
{
    ...
    public function initContent()
    {
	  ...
	  if ( isset( $_POST['mode'] )  )  // ADDED YGPC 
		{
			 $this->context->smarty->assign('mode', $_POST['mode'] );
			
			// ADDED YGPC to usw with Upay module 
			if( 0 !== (int)$this->context->customer->id )
			{
				$_SESSION['id']=  (int)$this->context->customer->id;
				$this->context->smarty->assign('connected', 1 );
			}
			else 
		 	 {
					$this->context->smarty->assign('connected', 0 );
			
			            return '<p class="warning">'.Tools::displayError('נא להתחבר').'</p>';
			 }          
		 }   //isset( $_POST['mode'] )
   }  //!$this->errors
